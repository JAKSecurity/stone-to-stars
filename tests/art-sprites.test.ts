...
describe('SPRITES registry', () => {
  ...
  it('Renaissance projectile + hero sprites are registered and valid', () => {
    const renaissanceSpriteIds = ['shot_musket', 'shot_pellet', 'shot_pistol', 'shot_grenade', 'hero_renaissance'];
    for (const id of renaissanceSpriteIds) {
      expect(SPRITES[id], `sprite ${id} should be registered`).toBeDefined();
      expect(validateSpriteDef(SPRITES[id]), `sprite ${id}: ${validateSpriteDef(SPRITES[id]).join('; ')}`).toEqual([]);
    }
  });
  ...
  it('Industrial projectile + hero sprites are registered and valid', () => {
    const industrialSpriteIds = ['shot_bullet', 'shot_fire', 'shot_dynamite', 'shot_spark', 'hero_industrial'];
    for (const id of industrialSpriteIds) {
      expect(SPRITES[id], `sprite ${id} should be registered`).toBeDefined();
      expect(validateSpriteDef(SPRITES[id]), `sprite ${id}: ${validateSpriteDef(SPRITES[id]).join('; ')}`).toEqual([]);
    }
  });

  it('Modern building sprites are registered and valid', () => {
    const modernBuildingIds = ['motor_pool', 'barracks', 'airfield'];
    for (const id of modernBuildingIds) {
      expect(SPRITES[id], `sprite ${id} should be registered`).toBeDefined();
      expect(validateSpriteDef(SPRITES[id]), `sprite ${id}: ${validateSpriteDef(SPRITES[id]).join('; ')}`).toEqual([]);
    }
  });

  it('Modern enemy sprites are registered and valid', () => {
    const modernIds = ['rifleman', 'halftrack', 'gunship', 'juggernaut'];
